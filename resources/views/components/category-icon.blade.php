@props(['icon' => null, 'size' => 'w-6 h-6'])

@php
    // Mapeo de iconos SVG (Heroicons outline)
    $icons = [
    ...
    $iconPath = $icons[$icon] ?? $icons['tag'];
@endphp

<svg {{ $attributes->merge(['class' => $size]) }} fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
    {!! $iconPath !!}
</svg>
